...
@Component({
  selector: 'app-voie-edit-admin',
  templateUrl: './voie-edit-admin.component.html'
})
export class VoieEditAdminComponent extends AbstractEditController<VoieDto, VoieCriteria, VoieService>   implements OnInit {
   ...
   public async openCreateStation(station: string) {
        const isPermistted = await this.roleService.isPermitted('Station', 'edit');
        if(isPermistted) {
             this.station = new StationDto();
