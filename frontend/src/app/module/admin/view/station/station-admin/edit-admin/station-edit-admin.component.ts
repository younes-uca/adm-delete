...
@Component({
  selector: 'app-station-edit-admin',
  templateUrl: './station-edit-admin.component.html'
})
export class StationEditAdminComponent extends AbstractEditController<StationDto, StationCriteria, StationService>   implements OnInit {
   ...
   public addVoies() {
       ...
       if (this.errorMessages.length === 0) {
            if(this.voiesElement.id == null){
                this.item.voies.push(this.voiesElement);
            }else{
                const index = this.item.voies.findIndex(e => e.id == this.voiesElement.id);
                this.item.voies[index] = this.voiesElement;
            }
   ...
   public async openCreateTelePeayage(telePeayage: string) {
        const isPermistted = await this.roleService.isPermitted('TelePeayage', 'edit');
        if(isPermistted) {
   ...
   public async openCreateTypeEquipement(typeEquipement: string) {
        const isPermistted = await this.roleService.isPermitted('TypeEquipement', 'edit');
        if(isPermistted) {
   ...
   public async openCreateTypeVoie(typeVoie: string) {
        const isPermistted = await this.roleService.isPermitted('TypeVoie', 'edit');
        if(isPermistted) {
